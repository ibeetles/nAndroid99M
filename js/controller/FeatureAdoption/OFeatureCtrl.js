    ...
    angular.module('ngAndroidNext99')
        .controller('ngOfeatureCtrl', function($scope, buganizerF) {
            ...
            angular.element(document).ready(function () {
                ...
                console.log('angular.element.ready - ngOfeatureCtrl - ' + currentdate.getMilliseconds());

                buganizerF.initIssueTrackerAPI().then(
                    // success function
                    function(authResult) {
                        console.log("succeeded in init API - initIssueTrackerAPI - OFeatureCtrl.js");
                        ...
                        buganizerF.getHotlistEntries('AndroidOFeatureAdoption').then(
                            ...
                            function (msg) {
                                console.log(msg);
                                console.log('failed to retrieve hotlist and its bug list - initIssueTrackerAPI - OFeatureCtrl.js');
                            }
                        );
                    },
                    //error function
                    function(authResult) {
                        console.log("failed in init API - initIssueTrackerAPI - OFeatureCtrl.js");
                    }
                );
            });

            function _parseData(response) {
                console.log('parsing data - _parseData - OFeatureCtrl.js');
                if(!response) {
                    console.log('Bad Query executed - _parseData - OFeatureCtrl.js');
                    return;
                }

                if (!response['hotlistEntries']) {
                    console.log('No data existing - _parseData - OFeatureCtrl.js');
                    return;
                }
